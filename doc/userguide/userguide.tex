...
If you're missing mysql, you should uncomment the line that sets the
\verb/no_mysql/ variable. That will disable building all modules that
require mysql, which is currently only the alias database. If you're
missing Berkeley DB, you should uncomment the line that sets
\verb/no_bdb/. This will disable the scoring modules and the
\verb/dbtool/. If you're missing Python, you should uncomment the line
that sets \verb/no_python/, which will disable the Python module loader.
...
There are currently
% grep 'EXPORT *int *MM_[a-z]' ~/src/asss/src/*.c | wc -l
69
